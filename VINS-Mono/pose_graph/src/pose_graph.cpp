...
void PoseGraph::registerPub(ros::NodeHandle &n)//发布轨迹path
{
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)//添加关键帧，完成回环检测和闭环的过程
{
    //cout<<"执行addKeyFrame()"<<endl;
    //shift to base frame
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    if (sequence_cnt != cur_kf->sequence)//创建一个新的图像序列
    {
        sequence_cnt++;
        sequence_loop.push_back(0);
        w_t_vio = Eigen::Vector3d(0, 0, 0);
    ...
    cur_kf->getVioPose(vio_P_cur, vio_R_cur);//获取当前帧位姿vio_P_cur,vio_R_cur并更新
	...
	int loop_index = -1;
    //将四个选定帧输出
    vector <int> loop_indexs;//修改


    if (flag_detect_loop)//1
    {
        TicToc tmp_t;
        loop_index = detectLoop(cur_kf, cur_kf->index);//回环检测候选帧，返回索引
        //loop_indexs = detectLoop1(cur_kf, cur_kf->index);//找到所有帧号的匹配帧，修改
    }
    else
    {
    ...
    // if (loop_indexs[0] != -1)
	// {  
    //     for  (unsigned int i = 0; i < loop_indexs.size(); i++)
    //     {
    //         //cout<<cur_kf->index<<"   "<<loop_indexs[i]<<endl; 
    //         //printf(" %d detect loop with %d \n", cur_kf->index, loop_index);
    //         KeyFrame* old_kf = getKeyFrame(loop_indexs[i]);
    //         bool a = cur_kf->drawConnection(old_kf);
    //     }
    // }

	if (loop_index != -1)//找对匹配帧
    //if (loop_indexs[0] != -1)
	{   
            ...
            if (cur_kf->findConnection(old_kf))
            {
                ...
                //计算回环位姿和VIO位姿之间的偏移量，纠正当前帧位姿w_P_cur,w_R_cur
            ...
            if (old_kf->sequence != cur_kf->sequence && sequence_loop[cur_kf->sequence] == 0)//若回环帧和当前帧不是同一个图像序列，则合并在世界坐标系下
            {  
                w_r_vio = shift_r;
                w_t_vio = shift_t;
                vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
            ...
            m_optimize_buf.lock();
            optimize_buf.push(cur_kf->index);//将当前帧放入到优化队列中
    ...
    path[sequence_cnt].poses.push_back(pose_stamped);//构建path并发布
...
vector<int> PoseGraph::detectLoop1(KeyFrame* keyframe, int frame_index)//输出多帧结果
{
    ...
    if (DEBUG_IMAGE)
    {
        ...
        cv::resize(keyframe->image, compressed_image, cv::Size(376, 240));//输入，输出，尺寸
    ...
    QueryResults ret;
    TicToc t_query;
    db.query(keyframe->brief_descriptors, ret, 4, frame_index - 50);//描述符，检测结果，结果个数，帧号必须小于此值
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)//回环检测
{
    ...
    {
        int feature_num = keyframe->keypoints.size();
        cv::resize(keyframe->image, compressed_image, cv::Size(376, 240));               //将当前帧压缩成compressed_image
        putText(compressed_image, "feature_num:" + to_string(feature_num), cv::Point2f(10, 10), CV_FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255));
        image_pool[frame_index] = compressed_image;
    }
    TicToc tmp_t;
    ...
    QueryResults ret;
    TicToc t_query;
    //查询字典，得到与每一帧的相似度评分ret
    db.query(keyframe->brief_descriptors, ret, 4, frame_index - 50);              //描述符，检测结果，结果个数，匹配帧必须小于此值    从数据库里找匹配帧
    //printf("query time: %f", t_query.toc());
    //cout << "Searching for Image " << frame_index << ". " << ret << endl;
    ...
    if (DEBUG_IMAGE)
    {
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            int tmp_index = ret[i].Id;
            auto it = image_pool.find(tmp_index);
            cv::Mat tmp_image = (it->second).clone();//从数据库里找到的历史关键帧
            putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50), CV_FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
            cv::hconcat(loop_result, tmp_image, loop_result);
        }
    }
    // a good match with its nerghbour确保与相邻帧具有良好的相似度评分
    //评分大于0.015的关键帧为回环候选帧，返回最早的关键帧索引值
    if (ret.size() >= 1 &&ret[0].Score > 0.05)
        for (unsigned int i = 1; i < ret.size(); i++)
        {
            //if (ret[i].Score > ret[0].Score * 0.3)
...
void PoseGraph::loadPoseGraph()//从file_path读取位姿图，下载历史地图信息，将历史中 的每一个关键帧的所有信息都存放在keyframe中
{
    TicToc tmp_t;
    FILE * pFile;
    string file_path = POSE_GRAPH_SAVE_PATH + "pose_graph.txt";
                                    ...
                                    &keypoints_num) != EOF) 
    {
        ...
        */
        ...
        if (DEBUG_IMAGE)//1
        {
            ...
            image = cv::imread(image_path.c_str(), 0);//rviz中的显示
        }
        ...
        for (int i = 0; i < keypoints_num; i++)//brief_descriptors，keypoints，keypoints_norm
        {
...
void PoseGraph::publish()//用于发布topic，将历史地图在rviz中画出
{
    for (int i = 1; i <= sequence_cnt; i++)
    {
...
void PoseGraph::updateKeyFrameLoop(int index, Eigen::Matrix<double, 8, 1 > &_loop_info)//跟新关键帧的回环信息
{
