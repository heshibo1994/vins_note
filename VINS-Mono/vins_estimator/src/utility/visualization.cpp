...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_keyframe_pose = n.advertise<nav_msgs::Odometry>("keyframe_pose", 1000);//发布pose_buf
    pub_keyframe_point = n.advertise<sensor_msgs::PointCloud>("keyframe_point", 1000);//发布point_buf
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    //cout<<"进入pubKeyFrame函数-----------------------------------------------进入pubKeyFrame函数"<<endl;
    //初始化成功之前solver_flag=INITIAL
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)

    //if (1)
    {

        //cout<<"pubKeyframe()===============pub camera pose, 2D-3D points of keyframe"<<endl;
        int i = WINDOW_SIZE - 2;
        //Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic[0];
        ...
        pub_keyframe_pose.publish(odometry);//发布数据：kf的pose信息
        ...
        pub_keyframe_point.publish(point_cloud);//发布数据：关键点的2d和3d信息
    }
}
