...
std::string POSE_GRAPH_SAVE_PATH;//init
...
void readParameters(ros::NodeHandle &n)
{
    ...
    fsSettings["pose_graph_save_path"] >> POSE_GRAPH_SAVE_PATH;//init
...
void initKeyFrame::getPose(Eigen::Vector3d &_T_w_i, Eigen::Matrix3d &_R_w_i)
{
    _T_w_i = T_w_i;
    _R_w_i = R_w_i;
}

initKeyFrame::initKeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, Vector3d &_T_w_i, Matrix3d &_R_w_i,
...
void saveMapToInit()
{
    ...
    list<initKeyFrame*> keyframelist;
    vector<vector<BRIEF::bitset>> initbrief;
    //BriefDatabase db;//存储描述符的数据库db
    string file_path = POSE_GRAPH_SAVE_PATH + "pose_graph.txt";
    printf("saveMapToInit: %s \n", file_path.c_str());
    printf("saveMapToInit loading...\n");
    pFile = fopen (file_path.c_str(),"r");
    if (pFile == NULL)
    {
        printf("saveMapToInit failed \n");
        return;
    }
    int index;
    double time_stamp;
                                    ...
                                    &keypoints_num) != EOF) 
    {
        ...
        initKeyFrame* initkeyframe = new initKeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index, loop_info, keypoints, keypoints_norm, brief_descriptors);
       // initKeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, Vector3d &_T_w_i, Matrix3d &_R_w_i,
        ...
        Vector3d P;
        Matrix3d R;
        initkeyframe->getPose(P, R);
        Quaterniond Q{R};
        geometry_msgs::PoseStamped pose_stamped;
        nav_msgs::Path base_path;
        pose_stamped.header.stamp = ros::Time(initkeyframe->time_stamp);
        ...
        keyframelist.push_back(initkeyframe);
        initbrief.push_back(initkeyframe->brief_descriptors);
        //db.add(initkeyframe->brief_descriptors);
        
        //loadKeyFrame(initkeyframe);
    }
    fclose (pFile);
    cout<<initbrief.size()<<endl;
    printf("saveMapToInit successful: %f s\n", tmp_t.toc()/1000);
}
